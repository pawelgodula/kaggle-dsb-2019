...
class TrainerLGBM:
    ...
    def set_tr_val_indexes(
        self, full_df: pd.DataFrame, n_fold: int
    ...
    def plot_importances(
        ...
        sns.barplot(
            x="Value",
            y="Feature",
            data=feature_imp.sort_values(by="Value", ascending=False).iloc[
                :n_top_feats, :
            ],
        )
    ...
    def validate_categoricals(
        self, full_df: pd.DataFrame, categoricals: List[str]
    ) -> pd.DataFrame:
        """
    ...
    def fit_kfold(
        ...
        categoricals: List[str] = None,
    ) -> Tuple[List[lgb.LGBMModel], np.ndarray, np.float64]:
        """
        ...
        for i, (tr_idx, val_idx) in enumerate(tr_val_idx):
            ...
            model = self.fit(
                train_df, val_df, task_type, params, features, target, eval_metric
            )
            if print_results:
    ...
    def find_unimportant_features(
        ...
        for model in models:
            ...
            zero_importance_features = feat_imp_df[feat_imp_df["Value"] == 0][
                "Feature"
            ].tolist()
    ...
    def optimize_hyperparameters(
        ...
        study = optuna.create_study(
            direction="maximize"
            if eval_metric in ["accuracy", "f1", "auc"]
            else "minimize"
        )
    ...
    def optimize_features_params(
        ...
        """
        unimportant_features = self.find_unimportant_features(
            full_df,
            task_type,
            params,
            features,
